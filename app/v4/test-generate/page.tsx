...
import { Sparkles, Image as ImageIcon, AlertCircle, RefreshCw, X, ChevronLeft, ChevronRight, ListOrdered, CheckCircle } from "lucide-react"
import type {
  Persona,
  BenchmarkPost,
  PostType,
...
export default function TestGeneratePage() {
  ...
  async function handleGenerate(includeImage: boolean) {
    ...
    try {
      // ─── テキスト生成 ───
      setPhase("text")
      const r1 = await fetch("/api/v4/generate-post", {
        method: "POST",
      ...
      // ─── 画像生成 ───
      setPhase("image")
      const r2 = await fetch("/api/v4/generate-image", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  async function handleEnqueue() {
    ...
    setEnqueuing(true)
    try {
      const r = await fetch("/api/v4/jobs/enqueue", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          personaId,
      ...
      setEnqueueToast("キューに追加しました。ヘッダーの「生成キュー」から進捗を確認できます。")
      setTimeout(() => setEnqueueToast(null), 5000)
    } catch (e) {
      setError(e instanceof Error ? e.message : "キュー追加失敗")
    } finally {
      setEnqueuing(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        {/* エンキュー成功トースト */}
        {enqueueToast && (
          <div className="flex items-center gap-2 text-sm px-3 py-2.5 rounded-xl" style={{ background: "#dcfce7", color: "#16a34a", border: "1px solid #86efac" }}>
            <CheckCircle className="w-4 h-4 flex-shrink-0" />
            <span>{enqueueToast}</span>
          </div>
        )}
        ...
        <div className="flex gap-3 flex-wrap pt-2">
          ...
          >
            {enqueuing ? (
              ...
              <><ListOrdered className="w-4 h-4" /> キューに追加（テキスト＋画像）</>
            )}
          </button>
          ...
          >
            {generating && phase === "text" ? (
              ...
              <><Sparkles className="w-4 h-4" /> テキストのみ（即時）</>
            )}
          </button>
        </div>
      </div>
