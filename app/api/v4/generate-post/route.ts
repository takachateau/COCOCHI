/**
 * POST /api/v4/generate-post
 * v4 単発投稿生成: ペルソナ × 型（自動選択）× 商品 で1投稿のテキストを生成する
 *
...
export async function POST(req: NextRequest) {
  try {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      if (!duplicate) break
      console.log(`[v4/generate-post] 被り検出 (attempt ${attempt + 1}): "${generated.overallTitle}" → 再生成`)
    ...
    console.error("[v4/generate-post]", e)
